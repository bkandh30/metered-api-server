...
pub fn validate_sensor_id(id: &str) -> Result<(), String> {
    if id.is_empty() {
    ...
    {
        return Err("Sensor ID contains invalid characters".to_string());
    }
...
pub fn validate_unit(unit: &str) -> Result<(), String> {
    ...
    {
        return Err("Unit contains invalid characters".to_string());
    }

    Ok(())
}

pub fn validate_value(value: f64) -> Result<(), String> {
    if value.is_nan() || value.is_infinite() {
        return Err("Value must be a valid number".to_string());
    }

    if value.abs() > 1_000_000.0 {
        return Err("Value exceeds maximum allowed range".to_string());
    }

    Ok(())
}

pub fn validate_api_key_name(name: &str) -> Result<(), String> {
    if name.is_empty() {
...
pub fn body_limit() -> impl Filter<Extract = (), Error = Rejection> + Clone {
    body::content_length_limit(MAX_BODY_SIZE).map(|_| ())
}
